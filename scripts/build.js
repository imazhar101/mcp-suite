...
function buildServer(serverName, progressBar, current, total) {
  ...
  try {
    ...
    // Run tsc from within the server directory with relative path
    ...
    progressBar.update(current + 1, `❌ ${serverName} failed: ${firstLine}`);
    return false;
  }
}
...
function buildShared() {
  try {
    ...
    const errorMessage = error.stderr || error.stdout || error.message;
    const firstLine = errorMessage.split('\n')[0];
    console.log(`❌ Shared modules failed: ${firstLine}`);
    return false;
  }
}
