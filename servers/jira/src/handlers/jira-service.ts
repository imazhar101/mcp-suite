...
export class JiraService {
  ...
  processDescription(description: string | ADFDocument | undefined): ADFDocument | undefined {
  ...
  async searchIssues(request: SearchIssuesRequest): Promise<ServerResponse<JiraIssue[]>> {
    try {
      ...
      const response = await this.client.get('/search', {
        params: {
          ...
          fields: 'summary,status,assignee,reporter,priority,issuetype,project,created,updated,description'
        }
      });
  ...
  async getIssue(issueKey: string): Promise<ServerResponse<JiraIssue>> {
    try {
      ...
      const response = await this.client.get(`/issue/${issueKey}`, {
        params: {
          fields: 'summary,status,assignee,reporter,priority,issuetype,project,created,updated,description,comment'
        }
      });
      ...
      const issueData: JiraIssue = {
        ...
        created: issue.fields.created,
        updated: issue.fields.updated,
        comments: issue.fields.comment?.comments?.slice(-5).map((comment: any) => ({
          id: comment.id,
  ...
  async createIssue(request: CreateIssueRequest): Promise<ServerResponse<{ key: string; url: string }>> {
    try {
      this.logger.info('Creating issue', { projectKey: request.projectKey, summary: request.summary });
  ...
  async updateIssue(request: UpdateIssueRequest): Promise<ServerResponse<void>> {
    try {
  ...
  async transitionIssue(issueKey: string, transitionName: string): Promise<ServerResponse<void>> {
    try {
      ...
      const transition = transitions.find(
        (t: JiraTransition) => t.name.toLowerCase() === transitionName.toLowerCase()
      );

      if (!transition) {
        const availableTransitions = transitions.map((t: JiraTransition) => t.name).join(', ');
        return {
          success: false,
  ...
  async addComment(issueKey: string, comment: string | ADFDocument, format: 'plain' | 'rich' = 'plain'): Promise<ServerResponse<{ id: string }>> {
    try {
      this.logger.info('Adding comment', { issueKey });
      
      const processedComment = typeof comment === 'string' ? this.textToADF(comment, format) : comment;
  ...
  async getIssueTransitions(issueKey: string): Promise<ServerResponse<JiraTransition[]>> {
    try {
      this.logger.info('Getting issue transitions', { issueKey });
      
      const response = await this.client.get(`/issue/${issueKey}/transitions`);
      const transitions: JiraTransition[] = response.data.transitions.map((transition: JiraTransition) => ({
        id: transition.id,
  ...
  async assignIssue(issueKey: string, assignee: string): Promise<ServerResponse<void>> {
    try {
