...
export class PuppeteerService {
  ...
  async launch(options: PuppeteerOptions = {}): Promise<PuppeteerResult> {
    try {
      ...
      const safeArgs =
        options.args?.filter(
          (arg) => !dangerousArgs.some((dangerous) => arg.includes(dangerous))
        ) || [];
    ...
    try {
      ...
      if (params.selector) {
        ...
        if (!element) {
          return {
            success: false,
            error: `Element with selector "${params.selector}" not found`,
          };
        }
  ...
  async select(params: SelectParams): Promise<PuppeteerResult> {
    try {
      ...
      const selectedValues = await this.page.select(
        params.selector,
        ...params.values
      );
  ...
  async hover(params: HoverParams): Promise<PuppeteerResult> {
    try {
      ...
      return {
        success: true,
        data: `Hovered over element: ${params.selector}`,
      };
    } catch (error) {
      return {
        success: false,
