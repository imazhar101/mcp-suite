...
export class DynamoDBService {
  ...
  async batchGetItems(
    ...
    if (response.Responses) {
      for (const [tableName, items] of Object.entries(response.Responses)) {
        result[tableName] = (items as any[]).map((item: any) =>
          unmarshall(item)
        );
      }
    }
  ...
  async batchWriteItems(
    requestItems: Record<
      string,
      { putRequests?: DynamoDBItem[]; deleteRequests?: DynamoDBItem[] }
    >
  ): Promise<void> {
    const marshalledRequestItems: Record<string, any> = {};

    for (const [tableName, { putRequests, deleteRequests }] of Object.entries(
      requestItems
    )) {
      const requests = [];
