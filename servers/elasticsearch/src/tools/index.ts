...
export const tools: McpTool[] = [
  // Connection and Health Tools
  {
    ...
    description: 'Test connection to Elasticsearch cluster and get basic cluster info',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description: 'Get simplified node statistics including CPU, memory, and disk usage',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
  ...
  // Index Management Tools
  {
    ...
    description: 'List all indices with basic information (health, status, document count, size)',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description: 'Get detailed information about a specific index (stats, mappings, settings)',
    inputSchema: {
      type: 'object',
      properties: {
        index: {
  ...
  // Search Tools
  {
    ...
    description: 'Search documents in an index with query, filters, sorting, and aggregations (max 1000 results)',
    inputSchema: {
      type: 'object',
      properties: {
        index: {
        ...
        _source: {
          description: 'Fields to include in results (true/false or array of field names)',
        },
        highlight: {
          type: 'object',
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        size: {
          type: 'number',
          description: 'Number of document hits to return (default 0 for aggregation-only)',
          maximum: 100,
        },
      },
      required: ['index', 'aggs'],
  ...
  // Bulk Operations
  {
    ...
    description: 'Perform multiple document operations in a single request (max 100 operations)',
    inputSchema: {
      type: 'object',
      properties: {
        index: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        query: {
          type: 'object',
          description: 'Elasticsearch query DSL to match documents for deletion',
        },
        refresh: {
          type: 'boolean',
          description: 'Whether to refresh the index after operation',
  ...
  // Advanced Operations
  {
    ...
    description: 'Copy documents from one index to another with optional query filter',
    inputSchema: {
      type: 'object',
      properties: {
        source_index: {
